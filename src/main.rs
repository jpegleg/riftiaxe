        ...
        loop {
            match self.server.accept() {
                Ok((socket, addr)) => {
                    ...
                    let timeo: DateTime<Utc> = Utc::now();
                    println!("[ {timeo:?} INFO ] - RIFTIAxe: {txid:?} - accepting new connection from {:?}", addr);
                ...
                Err(err) => {
                    let timerr: DateTime<Utc> = Utc::now();
                    let closix = env::var("txid");

                    println!(
                        "[ {timerr:?} ERRO ] - Error {closix:?} - encountered error while accepting connection; err={:?}",
                        err
                    );
                    return Err(err);
                }
    ...
    fn conn_event(&mut self, registry: &mio::Registry, event: &mio::event::Event) {
        ...
        if self.connections.contains_key(&token) {
            ...
            if self.connections[&token].is_closed() {
                let timei: DateTime<Utc> = Utc::now();
                let closid = env::var("txid");
                println!("[ {timei:?} INFO ] - RIFTIAxe: {closid:?} - closed connection with {:?}", event);
...
impl OpenConnection {
    ...
    fn close_back(&mut self) {
        if self.back.is_some() {
            ...
            let timedead: DateTime<Utc> = Utc::now();
            let tred = env::var("txid");

            match back.shutdown(net::Shutdown::Both) {
                Err(_) => {
                    println!("[ {timedead:?} ERRO ] - RIFTIAxe: {tred:?} - client session shutdown in error");
                    return;
                }
                Ok(_) => {
                    println!("[ {timedead:?} INFO ] - RIFTIAxe: {tred:?} - responded to client");
                    return;
                }
            };
        }
    ...
    fn do_tls_read(&mut self) {
        match self.tls_conn.read_tls(&mut self.socket) {
            Err(err) => {
                ...
                let timebo: DateTime<Utc> = Utc::now();
                let trex = env::var("txid");
                println!("[ {timebo:?} ERRO ] - RIFTIAxe: {trex:?} - read error {:?}", err);
                self.closing = true;
                return;
            }
            Ok(0) => {
    ...
    fn try_plain_read(&mut self) {
        if let Ok(io_state) = self.tls_conn.process_new_packets() {
            if io_state.plaintext_bytes_to_read() > 0 {
                ...
                let timefat: DateTime<Utc> = Utc::now();
                let closfat = env::var("txid");
                println!("[ {timefat:?} INFO ] - RIFTIAxe: {closfat:?} - bytes read {:?}", buf.len());
    ...
    fn try_back_read(&mut self) {
        ...
        if rc.is_err() {
            let timeff: DateTime<Utc> = Utc::now();
            let closff = env::var("txid");
            println!("[ {timeff:?} ERRO ] - RIFTIAxe: {closff:?} - backend read failed: {:?}", rc);
            self.closing = true;
            return;
        }
        ...
        match maybe_len {
            Some(0) => {
                let timefx: DateTime<Utc> = Utc::now();
                let closfx = env::var("txid");
                println!("[ {timefx:?} ERRO ] - RIFTIAxe: {closfx:?} - backend EOF");
                self.closing = true;
            }
            Some(len) => {
                self.tls_conn
    ...
    fn do_tls_write_and_handle_error(&mut self) {
        ...
        if rc.is_err() {
            let timefx: DateTime<Utc> = Utc::now();
            let closfx = env::var("txid");
            println!("[ {timefx:?} ERRO ] - RIFTIAxe: {closfx:?} - backend write: {:?} ", rc);
            self.closing = true;
        }
    }
...
Usage:
  riftiaxe --certs CERTFILE --key KEYFILE [--suite SUITE ...] \
...
fn main() {
    ...
    let timerz: DateTime<Utc> = Utc::now();
    println!("[ {timerz:?} INFO ] - RIFTIAxe: INIT - listening on {addr}");
    let mut poll = mio::Poll::new().unwrap();
    poll.registry()
        .register(&mut listener, LISTENER, mio::Interest::READABLE)
        .unwrap();

    let mode = ServerMode::Forward(args.arg_fport.expect("fport required"));
